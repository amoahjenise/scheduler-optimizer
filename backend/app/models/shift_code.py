...
class ShiftCode(Base):
    ...
    total_hours = Column(Float, nullable=False)  # Total clock time (e.g., 12.0)
    unpaid_break_hours = Column(Float, nullable=False, default=0.75)  # Unpaid break duration (e.g., 0.75)
    paid_hours = Column(Float, nullable=False)  # Paid duration = total_hours - unpaid_break_hours
    paid_multiplier = Column(Float, nullable=False, default=1.0)  # Overtime multiplier (1.0=standard, 1.5=time-and-a-half)
    shift_category = Column(String(50), nullable=False, default="Standard")  # e.g., "Standard Day", "12h", "Night"
